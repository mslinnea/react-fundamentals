...
This exercise uses PropTypes. The build doesn't fail if the props aren't correct, but there is a notice in the console.

PropTypes are reactive, while TypeScript is proactive.

`<>` is shorthand for `<React.Fragment>`  This allows a component to return multiple elements.  Therefore, we don't have to use a div to wrap the elements.
